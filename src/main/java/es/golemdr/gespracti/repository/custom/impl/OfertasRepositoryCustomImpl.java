...
public class OfertasRepositoryCustomImpl implements OfertasRepositoryCustom{

    @PersistenceContext
    private EntityManager em;

	@Transactional
	public List<Oferta> findOfertas(Oferta filtro, PaginacionBean paginacion) {

		List<Oferta> resultado = null;

		CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
		CriteriaQuery<Oferta> query = criteriaBuilder.createQuery(Oferta.class);
		Root<Oferta> oferta = query.from(Oferta.class);
		...
		query.select(oferta).where(criteriaBuilder.and(predicates.toArray(new Predicate[predicates.size()])));
